...
(*
let timeout =
  Lwt_unix.sleep 20. >|= fun () -> "Done with timeout"
;;
...
module Spotify : Spotify = struct
  ...
  let playlist_of_id (id: string) (api_token: string): playlist Lwt.t =
    let%lwt playlist_body = request_playlist_metadata id api_token in
    ...
    let features = batch_body |> Yojson.Safe.from_string |> playlist_features_yojson_to_matrix in
    Lwt.return {name; id; features;}

  (* waiting on campuswire, how to resolve the type error *)
  let song_of_id (id: string) (api_token: string): song Lwt.t =
    let%lwt features_body = request_song_features id api_token in
    let%lwt metadata_body = request_song_metadata id api_token in
    let name = get_field_remove_quotes "name" metadata_body in
    let features = features_body |> Yojson.Safe.from_string |> song_features_yojson_to_vector in
    Lwt.return {name; id; features;}
end